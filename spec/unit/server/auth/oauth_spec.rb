...
RSpec.describe MCP::Server::Auth::OAuth do
  ...
  let(:oauth_provider) { described_class.new(options) }
  ...
  describe '#initialize' do
    it 'sets provider properties' do
      ...
      expect(oauth_provider.instance_variable_get(:@token_expiry)).to eq(3600)
      expect(oauth_provider.instance_variable_get(:@jwt_secret)).to eq('test_jwt_secret')
      expect(oauth_provider.instance_variable_get(:@issuer)).to eq('test_issuer')
      expect(oauth_provider.instance_variable_get(:@scopes)).to contain_exactly('tools:read', 'tools:call')
    end
    
    it 'uses default values if not provided' do
      provider = described_class.new(client_id: 'client', client_secret: 'secret')
      
      expect(provider.instance_variable_get(:@token_expiry)).to eq(3600)
      expect(provider.instance_variable_get(:@jwt_secret)).to be_a(String)
      expect(provider.instance_variable_get(:@issuer)).to eq('mcp_server')
      expect(provider.instance_variable_get(:@scopes)).to eq(['mcp'])
    end
  end
  ...
  describe '#create_jwt' do
    let(:token) do
      OAuth2::AccessToken.new(
        OAuth2::Client.new('test_client', 'test_secret'),
        'access_token',
