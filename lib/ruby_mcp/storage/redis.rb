  ...
  module Storage
    ...
    class Redis < Base
      def initialize(options = {})
        ...
        @ttl = options[:ttl] || 86400  # Default 1 day TTL in seconds
      end
      ...
      def create_context(context)
        ...
        if get_context(context_id)
          raise Error, "Context with ID '#{context_id}' already exists"
        end
      ...
      def update_context(context)
        ...
        unless get_context(context_id)
          raise Error, "Context with ID '#{context_id}' does not exist"
        end
      ...
      def get_content(context_id, content_id)
        ...
        if encoding == "base64"
          content = content.unpack("m0").first
        end
